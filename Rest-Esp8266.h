...
class Handler {
public:
    typedef std::function< short(TArgs... args) > F0;

    Handler() {}
    Handler(const F0& _f) : f0(_f) {
    }
...
class RestRequest : public TRequest, public TResponse {
  public:
    ...
    RestRequest(TWebServer& _server) : server(_server) {}
...
class RestRequestHandler : public ::RequestHandler
{
  public:
    ...
    virtual bool canHandle(HTTPMethod method, String uri) { return true; }
    ...
    RestRequestHandler& on(const char *endpoint_expression, Rest::MethodHandler<RequestHandler> methodHandler ) {
...
namespace ArduinoJson {
  class Request {
    public:
      Request() { request = requestDoc.to<JsonObject>(); }
